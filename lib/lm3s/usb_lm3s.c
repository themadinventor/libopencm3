 ...
 *
 * \brief <b>libopencm3 LM3S Universal Serial Bus controller </b>
 *
 * The LM3S USB driver is integrated with the libopencm3 USB stack. You should
 * use the generic stack.
 *
 * To use this driver, tell the linker to look for it:
 * @code{.c}
 *	extern usbd_driver lm3s_usb_driver;
 * @endcode
 *
 * And pass this driver as an argument when initializing the USB stack:
 * @code{.c}
 * usbd_device *usbd_dev;
 * usbd_dev = usbd_init(&lm3s_usb_driver, ...);
 * @endcode
 *
 * <b>Polling or interrupt-driven? </b>
 *
 * The LM3S USB driver will work fine regardless of whether it is called from an
...
#define SYSCTL_IMC	SYSTEMCONTROL_IMC
#define SYSCTL_SRCR2	SYSTEMCONTROL_SRCR2
#define SYSCTL_RIS	SYSTEMCONTROL_RIS

#define MAX_FIFO_RAM	(4 * 1024)

const struct _usbd_driver lm3s_usb_driver;
 ...
 * @cond private
 */
static inline void lm3s_usb_soft_disconnect(void)
{
	USB_POWER &= ~USB_POWER_SOFTCONN;
}

static inline void lm3s_usb_soft_connect(void)
{
	USB_POWER |= USB_POWER_SOFTCONN;
}

static void lm3s_set_address(usbd_device *usbd_dev, uint8_t addr)
{
	(void)usbd_dev;
...
static void lm3s_ep_setup(usbd_device *usbd_dev, uint8_t addr, uint8_t type,
			  uint16_t max_size,
...
static void lm3s_endpoints_reset(usbd_device *usbd_dev)
{
	/*
...
static void lm3s_ep_stall_set(usbd_device *usbd_dev, uint8_t addr,
			      uint8_t stall)
{
	(void)usbd_dev;
...
static uint8_t lm3s_ep_stall_get(usbd_device *usbd_dev, uint8_t addr)
{
	(void)usbd_dev;
...
static void lm3s_ep_nak_set(usbd_device *usbd_dev, uint8_t addr, uint8_t nak)
{
	(void)usbd_dev;
	(void)addr;
	(void)nak;
...
static uint16_t lm3s_ep_write_packet(usbd_device *usbd_dev, uint8_t addr,
...
static uint16_t lm3s_ep_read_packet(usbd_device *usbd_dev, uint8_t addr,
...
static void lm3s_poll(usbd_device *usbd_dev)
{
...
static void lm3s_disconnect(usbd_device *usbd_dev, bool disconnected)
{
	(void)usbd_dev;
...
static usbd_device *lm3s_usbd_init(void)
{
	int i;
	...
	/* Software reset USB */
	SYSCTL_SRCR2 |= SYSCTL_SRCR2_USB0;
	for (i = 0; i < 1000; i++) {
		__asm__("nop");
	}
	SYSCTL_SRCR2 &= ~SYSCTL_SRCR2_USB0;
...
const struct _usbd_driver lm3s_usb_driver = {
	.init = lm3s_usbd_init,
	.set_address = lm3s_set_address,
	.ep_setup = lm3s_ep_setup,
	.ep_reset = lm3s_endpoints_reset,
	.ep_stall_set = lm3s_ep_stall_set,
	.ep_stall_get = lm3s_ep_stall_get,
	.ep_nak_set = lm3s_ep_nak_set,
	.ep_write_packet = lm3s_ep_write_packet,
	.ep_read_packet = lm3s_ep_read_packet,
	.poll = lm3s_poll,
	.disconnect = lm3s_disconnect,
