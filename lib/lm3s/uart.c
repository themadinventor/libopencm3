 ...
 *
 * \brief <b>libopencm3 LM3S Universal Asynchronous Receiver Transmitter</b>
 *
 * The LM3S UART API provides functionality for accessing the UART hardware of
 * the LM3S.
 *
 * Please see the individual UART modules for more details. To use the UART, the
 * uart.h header needs to be included:
 * @code{.c}
 *	#include <libopencm3/lm3s/uart.h>
 * @endcode
 *
 * @{
 */
...
{
	uint32_t clock = rcc_get_system_clock_frequency();
 ...
 *	uart_enable_interrupts(UART0, UART_INT_RX | UART_INT_TX);
 * @endcode
 *
 ...
 * enable them with one call. For example, to enable both the RX and TX
 * interrupts, pass (UART_INT_RX | UART_INT_TX)
 *
 ...
 * disable them with one call. For example, to disable both the RX and TX
 * interrupts, pass (UART_INT_RX | UART_INT_TX)
 *
 * @param[in] uart UART block register address base @ref uart_reg_base
 ...
 *
 * The UART on the LM3S can either be used with a single character TX and RX
