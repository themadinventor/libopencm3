 ...
 * \brief <b>libopencm3 LM3S Clock control API</b>
 *
 * The LM3S clock API provides functionaliity for manipulating the system clock,
 ...
 * frequency of 50MHz.
 * @code{.c}
 *	// A divisor of 8 gives us a clock of 400/8 = 50MHz
 *	#define PLLDIV_50MHZ 8
 *
 ...
 *	rcc_set_pll_divisor(PLLDIV_50MHZ);
 *
 ...
 *	lm3s_rcc_sysclk_freq = 50E6;
 * @endcode
 *
 * The same can be achieved by a simple call to high-level routines:
 * @code
 *	// A divisor of 5 gives us a clock of 400/8 = 50MHz
 *	#define PLLDIV_50MHZ 8
 *
 *	rcc_sysclk_config(OSCSRC_MOSC, XTAL_16M, PLLDIV_50MHZ);
 * @endcode
 *
 * @{
 */
 ...
 * @code
 *	extern uint32_t lm3s_rcc_sysclk_freq;
 * @endcode
 */
uint32_t lm3s_rcc_sysclk_freq = 16000000;
...
{
	while (!(SYSCTL_RIS & SYSCTL_RIS_PLLLRIS));
}
...
void rcc_change_pll_divisor(uint8_t pll_div400)
{
	...
	lm3s_rcc_sysclk_freq = (uint32_t)400E6 / pll_div400;
}
...
void rcc_sysclk_config(enum osc_src src, enum xtal_t xtal, uint8_t pll_div400)
{
	...
	if (pll_div400) {
		...
		switch (src) {
		case OSCSRC_PIOSC:
			lm3s_rcc_sysclk_freq = 16000000;
			break;
		case OSCSRC_PIOSC_D4:
			lm3s_rcc_sysclk_freq = 4000000;
			break;
		case OSCSRC_MOSC:
			lm3s_rcc_sysclk_freq = xtal_to_freq(xtal);
			break;
		case OSCSRC_32K_EXT:
			lm3s_rcc_sysclk_freq = 32768;
			break;
